angular.module('cp_app').controller('ExpenseDeclaration', function ($scope, $rootScope){
    ...
    $scope.getAccounts = function(){
        debugger;
        ApplicantPortal_Contoller.getProposalAccount($rootScope.userId,function(result,event){
            if (event.status && result != null) {
                debugger;
                $scope.accList = result.Account;
                $scope.$apply();
            }
        })
    }
        ...
        if($scope.expenseList.length>0){
            for(var i=0;i<$scope.expenseList.length;i++){
                if($scope.expenseList[i].Description__c!=undefined){
                    $scope.expenseList[i].Description__c=$scope.expenseList[i].Description__c.replace(/&amp;/g,'&').replace(/&#39;/g,'\'').replaceAll('&amp;amp;','&').replaceAll('&amp;gt;','>').replaceAll('&lt;','<').replaceAll('lt;','<').replaceAll('&gt;','>').replaceAll('gt;','>').replaceAll('&amp;','&').replaceAll('amp;','&').replaceAll('&quot;','\'');
                }
