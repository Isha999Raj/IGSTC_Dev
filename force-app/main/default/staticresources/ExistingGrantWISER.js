     ...
     ApplicantPortal_Contoller.getApplicantDetailsForGrantWISER($rootScope.contactId, function (result, event){
            if(event.status) {
                ...
                for(var i=0;i<$scope.input.length;i++){
                    ...
                    //         }
                    //         if($scope.input[i].Existing_Grants__r[j].End_Date__c != undefined){
                    //             $scope.input[i].Existing_Grants__r[j].End_Date__c   = new Date($scope.input[i].Existing_Grants__r[j].End_Date__c);
                    //         }
                    //     }
