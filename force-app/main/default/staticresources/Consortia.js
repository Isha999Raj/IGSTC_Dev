            ...
            if (event.status) {
                ...
                if (result == null || result.length == 0) {
                    $scope.allCoordinatorDetails.push({
                        ...
                        "Contacts":[{"FirstName":"","Proposals__c":$rootScope.projectId,"Campaign__c":$scope.campaigntype}],
                        "Proposals__c":$rootScope.projectId,
                    });
                    // $scope.allCoordinatorDetails.push({
                    //     "Name": "",
            ...
            if($scope.allPartners[i].Contacts != undefined){
                for(var j=0;j<$scope.allPartners[i].Contacts.length;j++){
                    if($scope.allPartners[i].Contacts[j].FirstName == undefined || $scope.allPartners[i].Contacts[j].FirstName == ""){
                        swal("info", "Please Enter Head of project.");
                    ...
                    if($scope.allPartners[i].Contacts[j].Department == undefined || $scope.allPartners[i].Contacts[j].Department == ""){
                        swal("info", "Please Enter Department.");
...
debugger
        ...
        ApplicantPortal_Contoller.insertCoordinatorsInformation($scope.allPartners,$scope.contactList, function(result, event){
            $("#btnPreview").html('<i class="fa-solid fa-check me-2"></i>Save and Next');
            $scope.successmessage="Co-Ordinators details have been saved successfully.";
             if(isCoordinator=='false'){
                $scope.successmessage="Project Partner details have been saved successfully.";
             }
            if(event.status){
             debugger;
             if(flag=='c'){
