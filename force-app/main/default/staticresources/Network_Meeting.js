                ...
                if ($scope.applicantDetails.length > 0) {
                    ...
                    for (var i = 0; i < $scope.applicantDetails.length; i++) {
                        ...
                        $scope.MeetingList.push({
                            ...
                            "meetingAgenda": $scope.applicantDetails[i].Meeting_Agenda__c.replace(/&amp;/g,'&').replace(/&#39;/g,'\'').replaceAll('&amp;amp;','&').replaceAll('&amp;gt;','>').replaceAll('&lt;','<').replaceAll('lt;','<').replaceAll('&gt;','>').replaceAll('gt;','>').replaceAll('&amp;','&').replaceAll('amp;','&').replaceAll('&quot;','\''),
    ...
    $scope.saveMeetingDetails = function () {
        ...
        ApplicantPortal_Contoller.saveMeetingDetailss(objData, $rootScope.projectId, function (result, event) {
            ...
            if (event.status) {
                swal({
                    title: "Success",
                    text: "Your Network Meeting detail has been saved successfully.",
                    icon: "success",
                    buttons: true,
                    dangerMode: false,
                }).then((willDelete) => {
