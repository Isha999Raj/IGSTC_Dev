                ...
                if ($scope.applicantDetails.length > 0) {
                    ...
                    for (var i = 0; i < $scope.applicantDetails.length; i++) {
                        ...
                        $scope.PIList.push({
                            ...
                            "title": $scope.applicantDetails[i].Title__c.replace(/&amp;/g,'&').replace(/&#39;/g,'\'').replaceAll('&amp;amp;','&').replaceAll('&amp;gt;','>').replaceAll('&lt;','<').replaceAll('lt;','<').replaceAll('&gt;','>').replaceAll('gt;','>').replaceAll('&amp;','&').replaceAll('amp;','&').replaceAll('&quot;','\''),
    ...
    $scope.saveDeliverables = function () {
        ...
        ApplicantPortal_Contoller.saveDeliverables(objData, $rootScope.projectId, function (result, event) {
            ...
            if (event.status) {
                swal({
                    title: "Success",
                    text: "Your PI Deliverables detail has been saved successfully.",
                    icon: "success",
                    buttons: true,
                    dangerMode: false,
                }).then((willDelete) => {
