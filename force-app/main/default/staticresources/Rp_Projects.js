angular.module('rp_app').controller('projects_ctrl', function($scope,$sce,$rootScope) {
    ...
    $scope.resetPassword=function(){
        ...
        else
        {
            ...
            ReviewerPortal_Controller.updateReviewerDetails($scope.objReviewer, function(result, event){
                if(event.status && result !=null){
...
$scope.getOnload();
    ...
    $scope.enableProjectDetails = function(projectId,stageFlag,stage,prop_stage,programme){
        debugger;
        // if(stage=="Submitted"){
        ...
        for(var i = 0; i<$scope.getAllProposalFromReviewer.length;i++){
            if($scope.getAllProposalFromReviewer[i].Id == projectId){
                $scope.projectDetails($scope.getAllProposalFromReviewer[i],prop_stage,programme);
            }
        }    
        //$scope.getProposalDocuments(projectId);                   
    }

    $scope.projectDetails = function(project,prop_stage,programme){
        debugger;
        $scope.flag=false;
    ...
    $scope.submitResponseDetail = function(param1){
        ...
        for(var i=0;i<$scope.QuestionTempList.length; i++){
            ...
            console.log($scope.qtList);
                console.log(param1)
        ReviewerPortal_Controller.getAllResponseLineItem( $scope.qtList, $scope.reviewerMapId, param1,$scope.ratingScore, function(result, event){
